...
water.lost.data <- read.csv("results/2.water_lost_data.csv", stringsAsFactors = F) %>% 
  rename(dap = DAP)

traits <- read.csv("results/1.Zoom.corrected_Plantcv_traits.csv", stringsAsFactors = F)

meta.cols <- c("plantbarcode", "plant.num", "genotype",
               "rep", "source", "dap", "timestamp")

# average frames
traits_meta <- traits %>% 
  select(snapshot.num, any_of(meta.cols)) %>% 
  distinct()

traits_avg <- traits %>% 
  group_by(snapshot.num) %>% 
  select(-frame) %>% 
  summarise_at(vars(-any_of(meta.cols)), mean, na.rm = T)

traits_avg.meta <- left_join(traits_avg, traits_meta, by = "snapshot.num") %>% 
...
all.data.dap <- all.data.timepoint %>%
  group_by(plantbarcode, dap) %>%
  filter(watering.job.num == max(watering.job.num))
...
# write out
write.csv(x = all.data.dap_model.traits, file = "results/3.water.and.cv.data.by.dap.csv", quote = FALSE, row.names = FALSE)
